...
def createImages(request):
    file = request.FILES.get('file')  # Use get() to avoid KeyError if 'file' is not in request.FILES
    ...
    if not user:
        ...
        if orig_img:
            ...
            vit.heads = torch.nn.Linear(in_features=768, out_features=len(class_names)).to(device)

            pretrained_vit_transforms = pretrained_vit_weights.transforms()
            vit.load_state_dict(torch.load('/home/sami/Downloads/FYP/Tahreem/Vit.pth'))

            # imgg = requests.get(cloudinary_url)
            # target_image = torchvision.io.read_image(imgg.content).type(torch.float32)
            img = Image.open(BytesIO(requests.get(cloudinary_url).content)).convert('RGB')
            # convert PIL image to torch tensor
            target_image = torch.tensor(np.array(img)).permute(2, 0, 1).float()
            ...
            with torch.inference_mode():
                ...
                target_image_pred = vit(target_image.to(device))
            target_image_pred_probs = torch.softmax(target_image_pred, dim=1)
            target_image_pred_label = torch.argmax(target_image_pred_probs, dim=1)
            predict = int(target_image_pred_label.item())
            # Predicton End
            generated_image_instance = GeneratedImage.objects.create(userId=user, link=cloudinary_url, type=predict, generated=0)
            generated_image_instance.save()
            return Response({"message": "Mission Completed!"})
        return Response({"message": "Mission Failed!"})
    else:
        return Response({ 'message': 'No file provided' })# GAN new Experiment with server

def generate(user, num,disease):
    if not user:
        raise NotFound("User not found")
    if num == 0:
    ...
    for _ in range(t_images):
        ...
        cloudinary_response = cloudinary.uploader.upload(img_bytes)
        cloudinary_url = cloudinary_response.get("secure_url")
        generated_image_instance = GeneratedImage.objects.create(userId=user, link=cloudinary_url, type=disease, generated=1)
        generated_image_instance.save()
    print(f'{num} images generated for diease {disease}')
#gan model
@api_view(['POST'])
def generateImages(request):
    ...
    user_instance = User.objects.get(id=userId)
    generate(user_instance, normal, disease=3)
    generate(user_instance, cnv,    disease=0)
    generate(user_instance, dme,    disease=1)
    generate(user_instance, drusen, disease=2)
